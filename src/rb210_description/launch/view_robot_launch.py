...
def generate_launch_description():
    ...
    bringup_dir = get_package_share_directory('rb210_description')
    launch_dir = os.path.join(bringup_dir, 'launch')

    # 因为 urdf文件中有一句 $(find dummy_moveit_config) 需要用xacro进行编译一下才行
    urdf_model_path = os.path.join(bringup_dir, f'urdf/rb210.urdf.xacro') # 修改这里
    ...
    declare_urdf_cmd = DeclareLaunchArgument(
        'urdf_file',
        default_value=os.path.join(bringup_dir, 'urdf', 'rb210.urdf.xacro'),
        description='Path to the URDF or Xacro file to load')
